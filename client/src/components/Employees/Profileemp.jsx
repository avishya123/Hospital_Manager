...
//   const { dept } = useParams();
  ...
  const [deptt, setDept] = useState('');
  const [discription, setDiscription] = useState('');
  const [_id, set_id] = useState('');

  useEffect(() => {
    axios.get(`http://localhost:5000/showemp/${id}`)
      .then(result => {
        console.log(result);
       ...
       <div style={{border:'2px solid black',width:'620px',textAlign:'center',fontSize:'25px',height:'700px',backgroundColor:'lightskyblue'}}>
        ...
        <h2  style={{border:'none',textAlign:'center',color:'blue',width:'500px',fontSize:'50px'}}>{name}</h2>
              <table style={{border:'none'}}>
                <tr>
              <td><label htmlFor="">Employee Name  :</label></td>
              <td>{name}</td><br />
              </tr>
              <tr>
             ...
             <tr>
              ...
              <td><Link to={`/profiledept/${deptt}`} style={{color:'blue'}}> <u>{deptt}</u></Link></td><br />
              </tr>
              <tr>
          ...
          <div><Link to={`/updateemp/${_id}`} className='btn btn-success bg-primary p-3'>Edit</Link>
</div>
        </form>
        </div>
    </div>
